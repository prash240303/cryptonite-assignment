...
import { fetchCoins } from '@/redux/slices/coins';
...
const ExplorePage: React.FC = () => {
  ...
  if (status === 'loading') {
    return <div className={`p-3 border-[2px] rounded-lg theme-transition ${isDarkMode ? "bg-gray-950 border-gray-600" : "bg-gray-100 border-gray-400"}`}>Loading...</div>;
  }
  
  if (status === 'failed') {
    return <div className={`p-3 border-[2px] rounded-lg theme-transition ${isDarkMode ? "bg-gray-950 border-gray-600" : "bg-gray-100 border-gray-400"}`}>Error: {error}</div>;
  }
          ...
          <tbody className="text-gray-500 font-light">
            {coins && coins.map((coin: CoinTypes) => (
              <tr key={coin.id} draggable onDragStart={(e) => handleDragStart(e, coin)} className={`theme-transition ${isDarkMode ? "hover:bg-gray-900" : "hover:bg-gray-200"} cursor-pointer`}>
                ...
                <td className="py-2 px-3 text-right">{coin.data ? formatPrice(coin.data.market_cap) : 'N/A'}</td>
